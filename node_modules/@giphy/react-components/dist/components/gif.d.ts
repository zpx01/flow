import { IGif, IUser } from '@giphy/js-types';
import { ElementType, SyntheticEvent } from 'react';
export declare const GRID_COLORS: string[];
export declare const getColor: () => string;
export declare type EventProps = {
    onGifVisible?: (gif: IGif, e?: SyntheticEvent<HTMLElement, Event>) => void;
    onGifSeen?: (gif: IGif, boundingClientRect: ClientRect | DOMRect) => void;
    onGifClick?: (gif: IGif, e: SyntheticEvent<HTMLElement, Event>) => void;
    onGifRightClick?: (gif: IGif, e: SyntheticEvent<HTMLElement, Event>) => void;
};
export declare type GifOverlayProps = {
    gif: IGif;
    isHovered: boolean;
    onClick?: (gif: IGif) => void;
};
declare type GifProps = {
    gif: IGif;
    width: number;
    height?: number;
    backgroundColor?: string;
    className?: string;
    user?: Partial<IUser>;
    overlay?: ElementType<GifOverlayProps>;
    hideAttribution?: boolean;
    noLink?: boolean;
    borderRadius?: number;
    tabIndex?: number;
    style?: any;
};
declare type Props = GifProps & EventProps;
declare const Gif: {
    ({ gif, width, height: forcedHeight, onGifRightClick, className, onGifClick, onGifSeen, onGifVisible, user, backgroundColor, overlay, hideAttribution, noLink, borderRadius, style, tabIndex, }: Props): JSX.Element;
    className: string;
    imgClassName: string;
    imgLoadedClassName: string;
};
export default Gif;
